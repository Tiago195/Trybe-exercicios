...
const createMenu = (alimentos) => {
  const objeto = {
    fetchMenu: () => alimentos,
    consumption: [],
    // order: tutu,
    order(e) { this.consumption.push(e); },
    pay() {
      let teste = 0;
      for (let i = 0; i < this.consumption.length; i += 1) {
        if (Object.values(alimentos)[0][this.consumption[i]]) {
          teste += Object.values(alimentos)[0][this.consumption[i]];
        }
        if (Object.values(alimentos)[1][this.consumption[i]]) {
          teste += Object.values(alimentos)[1][this.consumption[i]];
        }
      }
      return teste;
    },
...
// console.log(meuRestaurante); // Retorno: undefined
